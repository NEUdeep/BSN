...
def PGM_proposal_generation(opt):
    ...
    for tid in range(opt["pgm_thread"] - 1):
        # tid = int(tid)
        # print(type(tid))
        tmp_video_list = list(video_list)[tid * num_videos_per_thread:(tid + 1) *
    ...
    tmp_video_list = list(video_list)[(opt["pgm_thread"] - 1) *
                                num_videos_per_thread:]
    p = mp.Process(target=generateProposals,
                   args=(
                       opt,
...
def PGM_feature_generation(opt):
    ...
    num_videos = len(video_list)
    num_videos_per_thread = num_videos / opt["pgm_thread"] # python2 a/b will got float.
    processes = []
    for tid in range(opt["pgm_thread"] - 1):
        tmp_video_list = video_list[tid * int(num_videos_per_thread):(tid + 1) *
                                    int(num_videos_per_thread)]
